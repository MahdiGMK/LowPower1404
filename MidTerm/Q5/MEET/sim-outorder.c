 ...
 *
 * $Id: sim-outorder.c,v 1.2 2000/11/29 21:01:57 cu-cs Exp $
 *
...
/* mostafa: start the program at this address. >0 */
static unsigned int initial_pc;

/* mostafa: address of end of program. >0 */
...
/* total number of Flash loads committed */
static counter_t sim_num_flash_loads = 0;
...
"latency of all pipeline operations.\n"
  ...
  /* initial address */
  opt_reg_uint(odb, "-initial:pc", "start address",
	       &initial_pc, /* default */0,
	       /* print */TRUE, /* format */NULL);
  /* end address */
  opt_reg_uint(odb, "-finish:pc", "exit address",
	       &finish_pc, /* default */0,
	       /* print */TRUE, /* format */NULL);
  ...
  /* trace options */
   ...
   opt_reg_flag(odb, "-profile", "look for areas with significant IPCs",
 	       &profile_mode, /* default */FALSE,
 	       /* print */TRUE, /* format */NULL);
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  stat_reg_counter(sdb, "sim_num_loads",
		   "total number of SRAM loads committed",
  ...
  stat_reg_formula(sdb, "sim_num_stores",
		   ...
		   "sim_num_refs - (sim_num_loads + sim_num_flash_loads)", NULL);
  ...
  /* add profiling stats */
    if (profile_mode){
      stat_reg_float(sdb, "MAX_IPC", "The Maximum IPC",
                   &max_ipc_value, /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "MAX_IPC", "the first cycle of the maximum IPC",
                   &max_ipc_time, /* initial value */0, /* format */NULL);
      stat_reg_float(sdb, "MIN_IPC", "The Minimum IPC",
                   &min_ipc_value, /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "MIN_IPC", "the first cycle of the minimum IPC",
                   &min_ipc_time, /* initial value */0, /* format */NULL);
      stat_reg_float(sdb, "MAXM_IPC", "Maximum IPC(insn>10000000)",
                   &maxm_ipc_value, /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "MAXM_IPC", 
                   "the cycle (insn>10000000)of the maximum IPC",
                   &maxm_ipc_time, /* initial value */0, /* format */NULL);
      stat_reg_float(sdb, "MINM_IPC", "Minimum IPC(insn>10000000)",
                   &minm_ipc_value, /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "MINM_IPC", 
                   "the cycle (insn>10000000)of the minimum IPC",
                   &minm_ipc_time, /* initial value */0, /* format */NULL);


      stat_reg_float(sdb, "range1", "Greatest IPC delta ",
                   &range_val[0], /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "range1", 
                   "start 10000 cycle range with greatest IPC delta ",
                   &range[0], /* initial value */0, /* format */NULL);

      stat_reg_float(sdb, "range2", "2nd greatest IPC delta ",
                   &range_val[1], /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "range2", 
                   "start 10000 cycle range with 2nd greatest IPC delta ",
                   &range[1], /* initial value */0, /* format */NULL);

      stat_reg_float(sdb, "range3", "3rd greatest IPC delta ",
                   &range_val[2], /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "range3", 
                   "start 10000 cycle range with 3rd greatest IPC delta ",
                   &range[2], /* initial value */0, /* format */NULL);

      stat_reg_float(sdb, "range4", "4th greatest IPC delta ",
                   &range_val[3], /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "range4", 
                   "start 10000 cycle range with 4th greatest IPC delta ",
                   &range[3], /* initial value */0, /* format */NULL);

      stat_reg_float(sdb, "range5", "5th greatest IPC delta ",
                   &range_val[4], /* initial value */0, /* format */NULL);
      stat_reg_int(sdb, "range5", 
                   "start 10000 cycle range  with 5th greatest IPC delta ",
                    &range[4], /* initial value */0, /* format */NULL);
  }
  ld_reg_stats(sdb);
...
*/
#define STORE_OP_READY(RS)						\
  ((RS)->idep_ready[0])
#define STORE_ADDR_READY(RS)						\
  ((RS)->idep_ready[1] && (RS)->idep_ready[2] && (RS)->idep_ready[3])
	  ...
	  if (verbose)
	    {
	      ...
	      myfprintf(stderr, "%10n @ 0x%08p: ",
			sim_ret_insn, LSQ[LSQ_head].PC);
	      md_print_uop(LSQ[LSQ_head].op,
			   LSQ[LSQ_head].IR, LSQ[LSQ_head].PC, stderr);
	  ...
	  if (verbose)
	    {
	      ...
	      myfprintf(stderr, "(%n) %10n {%d} @ 0x%08p: ",
			sim_cycle, sim_ret_insn,
			RUU[RUU_head].seq, RUU[RUU_head].PC);
	      md_print_uop(RUU[RUU_head].op,
			   RUU[RUU_head].IR, RUU[RUU_head].PC, stderr);
...
ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  while (RUU_index != branch_index)
	...
	panic("RUU head and tail broken");
	  ...
	  LSQ_num > 0 && (LSQ[LSQ_index].seq > RUU[RUU_index].seq))
	{
...
#if 0
      if (br_taken && in_flow)
	panic("branch taken in flow");
#endif
      if (!in_flow
 	  && (jmp_flow
 	      ||(pred_PC != regs.regs_NPC && (pred_perfect || op == MOV))
...
#include "machine.def"
  ...
  for (;;)
	...
	//printf("PC2=%x\r\n",regs.regs_PC);
      /* finish early? */
      if ((max_insts && sim_num_insn >= max_insts) || (finish_pc && regs.regs_PC==finish_pc))
	return;
    }
}
