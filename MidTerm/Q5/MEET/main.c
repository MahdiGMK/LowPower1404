...
static void signal_sim_stats(int sigtype) { sim_dump_stats = TRUE; }

/* exit signal handler */
static void signal_exit_now(int sigtype) { sim_exit_now = TRUE; }
...
static void banner(FILE *fd, int argc, char **argv) {
    ...
    fprintf(fd,
            "%s: Energy profiler tool based on SimpleScalar/%s Tool Set "
            "version %d.%d of %s.\n"
            "Developed by Mostafa Bazzaz at 2011.\n\n"
            "Copyright (c) 1994-2000 by Todd M. Austin.  All Rights Reserved.\n"
            "This version of SimpleScalar is licensed for academic "
            "non-commercial use only.\n"
            "\n",
            ((s = strrchr(argv[0], '/')) ? s + 1 : argv[0]), VER_TARGET,
...
int main(int argc, char **argv, char **envp) {
    ...
    opt_reg_flag(sim_odb, "-h", "print help message", &help_me,
                 /* default */ FALSE, /* !print */ FALSE, NULL);
    opt_reg_flag(sim_odb, "-v", "verbose operation", &verbose,
                 /* default */ FALSE, /* !print */ FALSE, NULL);
#ifdef DEBUG
    opt_reg_flag(sim_odb, "-d", "enable debug message", &debugging,
                 /* default */ FALSE, /* !print */ FALSE, NULL);
#endif /* DEBUG */
    opt_reg_flag(sim_odb, "-i", "start in Dlite debugger", &dlite_active,
                 /* default */ FALSE, /* !print */ FALSE, NULL);
    opt_reg_int(sim_odb, "-seed",
                "random number generator seed (0 for timer seed)", &rand_seed,
                /* default */ 1, /* print */ TRUE, NULL);
    ...
    opt_reg_string(sim_odb, "-chkpt",
                   "restore EIO trace execution from <fname>", &sim_chkpt_fname,
    ...
    opt_reg_int(sim_odb, "-nice", "simulator scheduling priority",
                &nice_priority,
